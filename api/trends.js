  ...
  return items
    .map((item, index) => {
      ...
      const score = Math.min(
        99,
        Math.round(
          48 + Math.log10(engagement + 1) * 14 + Math.min(20, velocity / 3),
        ),
      );
      return {
        id: item.id,
        ...
        score,
        growth: `传播指数 ${Math.max(1, Math.round(velocity))}`,
        signal: Math.max(1, Math.round(velocity)),
        engagement,
        time:
          recencyHours < 1
            ? `${Math.max(1, Math.round(recencyHours * 60))} 分钟前`
            : `${Math.round(recencyHours)} 小时前`,
        tone: score > 88 ? "快速升温" : score > 78 ? "持续上行" : "值得关注",
...
async function hackerNews() {
  ...
  return stories
    ...
    .map((x) => ({
      ...
      source: "Hacker News",
      engagement: (x.score || 0) + (x.descendants || 0) * 2,
      createdAt: new Date(x.time * 1000).toISOString(),
    }));
}

async function v2ex() {
  const data = await fetchJson("https://www.v2ex.com/api/topics/hot.json", {
    headers: { "User-Agent": "wechat-pulse/1.0" },
  });
  return data
    .slice(0, 12)
    .map((x) => ({
      ...
      title: x.title,
      url: x.url,
      source: `V2EX · ${x.node?.title || "热门"}`,
      engagement: (x.replies || 0) * 4 + 20,
      createdAt: new Date((x.last_touched || x.created) * 1000).toISOString(),
    }));
}
...
export default async function handler(req, res) {
  try {
    ...
    json(res, 200, {
      ...
      topics,
      sources: settled.map((x, i) => ({
        name: i ? "V2EX" : "Hacker News",
